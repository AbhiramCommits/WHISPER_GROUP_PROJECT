...
print("Loading embedding weights from npy files...")
TOKEN_EMBEDDING = np.load("token_embedding.npy")       # (51865, 384)
POS_EMBEDDING   = np.load("positional_embedding.npy")  # (448, 384)
...
def embed_token(token_id, position):
    """Token + positional embedding lookup → (1, 1, 384) float32."""
    emb = TOKEN_EMBEDDING[token_id] + POS_EMBEDDING[position]
    return emb.astype(np.float32)[np.newaxis, np.newaxis, :] 
...
for i, item in enumerate(ls_dataset):
    ...
    try:
        ...
        mel = log_mel_spectrogram(raw_audio)[np.newaxis, :, :] 
        sentence = item["text"]
